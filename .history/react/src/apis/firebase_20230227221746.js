...
export async function getAllMeetData(){
  ...
  return get(meetRef)
    .then((snapshot) => {
    if(snapshot.exists()){
      const snapshotVal = Object.values(snapshot.val());
      ...
      // const result = snapshotVal.map((meet) => {
      //   const year = meet.meetTime.split('-')[0];
      //   const month = meet.meetTime.split('-')[1];
      //   const date = meet.meetTime.split('-')[2];
