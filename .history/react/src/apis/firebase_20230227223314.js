...
export async function getAllMeetData(){
  ...
  return get(meetRef)
    ...
    if(snapshot.exists()){
      ...
      const test = result.map((meet) => {
        console.log(meet);
        const year = parseInt(meet.meetTime.date.split('-')[0]);
        const month =parseInt(meet.meetTime.date.split('-')[1]);
        const date =parseInt(meet.meetTime.date.split('-')[2]);
        console.log(year, month, date);
        console.log(currentYear, currentMonth, currentDate);
        console.log(year >= currentYear && month >= currentMonth && date >= currentDate);
