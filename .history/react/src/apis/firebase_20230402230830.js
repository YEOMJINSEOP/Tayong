...
export async function getCurrentUserState() {
  const user = await new Promise((resolve, reject) => {
    const unsubscribe = auth.onAuthStateChanged((user) => {
      ...
      resolve(user ? { ...user } : null); // create a new object with the user properties, or null if user is null
    }, reject);
  });
  return user ? { ...user } : null; // create a new object with the user properties, or null if user is null
}
