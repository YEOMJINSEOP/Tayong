...
function Form(props) {
  ...
  const validateMeet = () => {
    if(!meet.host){
      console.alert('host가 설정되지 않았습니다.');
      return false
    }
    if(!meet.meetId){
      console.alert('meetId is null');
      return false
    }
    if(meet.meetTime.date === 0 || meet.meetTime.time === 0){
      console.alert('meetTime undefined');
      return false
    }
    if(!meet.transport){
      console.alert('transport undefined');
      return false
    }
    if(meet.title.trim().length === 0){
      console.alert('제목을 설정하세요');
      return false
    }
    if(meet.content.trim().length === 0){
      console.arlert('내용을 입력하세요');
      return false
    }
    return true;
  }
